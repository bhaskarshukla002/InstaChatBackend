...
import com.example.InstaChat.mail.EmailService;
import com.example.InstaChat.mail.EmailTemplateName;
import com.example.InstaChat.user.UserRepository;
import com.example.InstaChat.auth.model.Token;
...
//                .orElseThrow(() -> new IllegalStateException("ROLE USER was not initiated"));
        if(userRepository.findByEmail(request.getEmail()).isPresent()){
            throw new MessagingException("User Already Present");
        }
...
//        sendValidationEmail(user);
    ...
    @Transactional
    public void activateAccountTest(String token) throws MessagingException {
        Token savedToken = tokenRepository.findByToken(token)
                // todo exception has to be defined
                .orElseThrow(() -> new RuntimeException("Invalid token"));
        if (LocalDateTime.now().isAfter(savedToken.getExpiresAt())) {
